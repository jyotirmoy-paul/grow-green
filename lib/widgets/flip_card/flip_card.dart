...
class FlipCardState extends State<FlipCard> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    controller?.addListener(() {
      ...
      if (value > 0.5 && _hideBackChild == true) {
        _hideBackChild = false;
        setState(() {});
      }

      if (value < 0.5 && _hideBackChild == false) {
        _hideBackChild = true;
        setState(() {});
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final child = Stack(
      ...
      children: <Widget>[
        ...
        _hideBackChild ? const SizedBox.shrink() : backPositioning(_buildContent(front: false)),
      ],
    );
